...
private _isPlayerHasSalvageKit = 'vn_b_item_toolkit' in (Items player);  //'vn_b_item_toolkit' in ((vestItesm player) + (uniformItems player) + (backpackItems player)); or "'vn_b_item_toolkit' in (backpackItems player)";
...
private _isPlayerExplosiveSpecialist = "player getUnitTrait 'explosiveSpecialist'";
private _isPlayerHasSalvageKit = 'vn_b_item_toolkit' in (Items player); //"'vn_b_item_toolkit' in (backpackItems player)";
private _isInRangeOfSalvage = "player distance cursorObject < 5";
private _isPlayerHasMD = 'MineDetector' in ((vestItems player) + (uniformItems player) + (backpackItems player));
private _isPlayerHasCharge = 'vn_mine_limpet_01_mag' in ((vestItems player) + (uniformItems player) + (backpackItems player));
...
private _codeOnStart = {
	...
	private _playerHasSalvageKit = 'vn_b_item_toolkit' in (Items player); //(backpackItems player);
	private
	private
	private
	private
...
private _codeOnTick = {};
private _codeOnComplete = {_charge = createMine ["SatchelCharge_F", player, [], 0]; 
player addOwnedMine _charge; _charge attachTo [cursorTarget,[0,0,-1]];
